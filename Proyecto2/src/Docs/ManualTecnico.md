...
Al ejcutar el comando, se creará la carpeta `node_modules`, la cuál tendrás las dependencias necesarias para levantar el frontend y que sea capaz de renderizar los componentes.
...
Para la creación de los pdf´s de reportes, se utilizan la librerías
...
La librería `jspdf` se utiliza para la creación del pdf, utiliza la declaración `const doc = new jsPDF();` para definir un nuevo objeto de la librería correspondiente, indisplensable para agregar los elemnetos que lo conformarán. Para la tabla se utiliza el código siguiente, se definen las columnas y filas, a las que se le agrega la data correspondiente, que viene desde la solicitud al servidor.
...
Con el plugin de la librería `jspdf-autotable` se crea la tabla, agregando elementos de la misma, tales como las columnas, las filas, el tamaño de letra y el color.
...
Para descargar el documento generado, se utiliza esta línea, la propiedad `save()` del objeto `doc` que recibe como parámetro el nombre deseado.
...
La clase `CodeGenerator` es la encargada de generar el código en `ARM64`, etsa está construida por una clase de php, a la cuál se le extiene la clase `GrammarBaseVisitor`.

~~~php
class CodeGenerator extends \GrammarBaseVisitor
~~~

Además, necesita la importación de los diferentes contextos de las clases generadas durante el análisis sintáctico y semántico
...
Para las diferentes secciones del código `ARM64`, se utilizan arreglos, donde cada posición del mismo es una línea de instrucción a imprimir, por lo que el primer paso es definirlas.
...
Se inicia definiendo la sección `.data`, fundamental para la definición de variables.
...
Luego la sección `.text` se define con valores por defecto, que siempre serán útiles para la ejecución del programa, como la alineación `.align 2` para manejar la memoria de manera eficiente, y la definición de la función `.global _start`, esto para decirle al programa donde inicar, el equivalente a la definción de la función main.
...
Para la construcción del código final, se utiliza la función `construirCodigo()`, esta utiliza el código almacenado en los arreglos definidos con anterioridad y que a lo largo de la ejecución del programa se fueron llenando con la data ingresada por el archivo analizado.
...
Para la impresión, se utilizan llamadas al systema `syscall`, específicamente la `syscall 0`, este es la designada por Linux para imprimir por consola, para ello se deben seguir reglas específicas, como almacenar en `x0` la dirección donde se encuentra el dato a imprimir, en `x1` la longitud del mensaje y se ejecuta con la expresión `svc #0`, que significa llamado al administrador.
...
Para el manejo individual de valores de tipo diferente a string, se crean funciones únicas para el manejo de la impresión.
...
Por ejemplo, para la impresión de boolenaos, se utiliza la función `emitirImprimirBool`, esta define una secuencia específica para poder imprimir booleanos de manera correcta, utilizando instrucciones como `add` o `b` para manejar las direcciones e imprimir de manera correcta la data solicitada.
